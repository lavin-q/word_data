...
public class GetWordJson {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			String input = readTxt(list.get(i), "UTF-8"); 
			
			String word = list.get(i).substring(list.get(i).lastIndexOf("\\")+1, list.get(i).length());
			...
			if(result != null && result.length() > 0){
				...
				String output = analyJson(resultMap,list.get(i));
				
				String fileName = "\\"+ word + ".json";
				
				outputFile(outpath + filePath + fileName, output, "UTF-8");
		    }
		}
